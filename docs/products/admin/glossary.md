...
Provider、Provider Origin、Channel 与 Route 的定义属于
[Gateway 词汇表](../gateway/glossary.md)和已接受的
[Gateway ADR-0001](../gateway/decisions/adr-0001-domain-terminology.md)。本表只定义 Admin
在这些共享对象之上的运营概念。
...
| 客观运行事实 | 可直接追溯到当前硬门禁、检测、运行态或历史记录的事实，例如 breaker、错误率、流式 TTFT、容量与最终权重；不是主观健康分数。 |
| 经营驾驶舱 | 面向内部运营的分层经营视图：决策层、分析中心和实时监控页。 |
| 归档 | 将 Provider、Provider Origin、Channel 或线路转为保留历史关联、默认不在在用列表显示的状态；不是硬删除。 |
