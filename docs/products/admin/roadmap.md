...
| 结果 | 指标 | 负责人 | 依赖 | 状态 |
| --- | --- | --- | --- | --- |
| 核验 Provider、Provider Origin、Channel 的管理模型是否已符合 ADR-0002 | 术语、页面和 API 均不混用“端点”与上游源站 | 管理后台团队、网关团队 | [ADR-0002](decisions/adr-0002-provider-origin-management.md) | 待核验 |
...
| 结果 | 指标 | 负责人 | 依赖 | 决策门槛 |
| --- | --- | --- | --- | --- |
| 完成归档、恢复与线路绑定迁移体验 | 归档保留历史，恢复落入受控状态，线路归档不遗留绑定 Key | 管理后台团队 | [运营管理](features/operations-management.md) | 实现与迁移核验 |
