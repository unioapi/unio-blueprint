...
- Provider 当前 USD 余额和低余额数量作为独立时点事实展示：未设置、正常、低余额、负余额四态可区分，不随经营
  时间范围变化；余额只供观察、对账和排查，不参与路由、fallback、breaker 或 Provider/Channel 状态。
- Provider 账本展示可靠请求成本、可靠模型探测成本和手工调额。请求消费必须关联请求、最终 attempt、成本快照、
  Channel 和模型；探测消费关联独立探测记录，不伪造客户请求。
- partial estimate、usage 不可靠或无成本快照的请求，以及已收到 2xx 但没有可靠 usage 的模型探测，不产生自动消费流水；
  它们进入独立的待对账成本风险视图，估算金额不并入余额或经营利润。明确失败的模型探测只保留探测事实。
  最终余额调额会收口调额前的同币种风险。
...
| 缓存贡献 | 作为估算展示 | 不并入账本利润。 |
| Provider 余额未设置 | 显示未设置，不视为零余额或低余额 | 首次消费或人工调额后建立余额。 |
| Provider 负余额 | 显示负余额，不改变流量 | 运营人员查看账本并按确认结果调额。 |
