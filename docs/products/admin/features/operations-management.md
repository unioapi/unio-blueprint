...
| 运营人员 | 发现并绑定渠道支持的模型 | 不能靠逐个猜测模型名来建立供给，也不能把列表发现误当成可调用验证。 |
...
- 把上游模型发现、参考目录采纳、本地绑定、逐模型验证和人工启用组织为一个可重复工作流。
...
8. 新建 Channel 固定为 disabled，创建后进入模型清单完成上游发现、匹配或采纳、停用绑定、逐模型验证和人工启用。
   已有 Channel 使用同一入口重新发现和对账，不依赖重新创建；上游不支持列表时保留手工绑定兜底。
...
- 上游模型发现与客户 `/v1/models` 无关。发现只维护成功快照和运行历史，不创建模型或绑定，不改变
  `credential_valid`、Channel、Model、绑定或 Route 状态；发现失败继续展示最近成功快照。
- 渠道模型清单合并成功快照与全部现有绑定；“本次未发现”只作为运营提示，不能自动停用或删除绑定。
- 本地精确模型、已采纳参考模型、未采纳参考目录候选和无匹配是不同处理路径。参考模型“采纳并绑定”必须原子
  完成，普通批量绑定也必须原子完成；所有新绑定固定为 disabled。
- 模型状态、Channel 状态、绑定状态和逐模型验证结果必须分开展示。验证成功不自动启用；启用绑定或替换上游
  模型名必须使用当前 Channel/Provider revisions 下同一模型映射的成功验证证据。
...
| 模型发现失败 | 保留最近成功快照和现有绑定，不改变凭据或运行状态 | 修复网络、凭据或端点后重新发现；不支持列表时手工绑定。 |
| 成功快照 revision 过期 | 标明快照已过期，不用它证明当前配置 | 对当前配置重新发现。 |
| 发现模型尚未绑定 | 展示本地精确匹配、参考目录候选或无匹配 | 绑定已有模型、采纳并绑定，或手工选择。 |
| 绑定已停用且验证成功 | 仍不参与路由 | 运营人员确认后显式启用绑定。 |
...
- [x] 新建和已有 Channel 共用模型清单；发现失败保留上次成功快照，上游不支持列表时可手工绑定。
- [x] 参考模型采纳与绑定原子完成；新 Channel 和新绑定固定停用，逐模型验证通过后仍需人工启用。
