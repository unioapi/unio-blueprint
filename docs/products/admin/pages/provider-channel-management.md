...
description: 运营人员维护 Provider API Root、Channel、模型供给、凭据、运行态与显式归档生命周期的页面设计。
status: draft
owner: 管理后台团队
...
related:
  ...
  - ../../gateway/features/channel-model-inventory.md
...
- 用受控的归档和恢复替代误删除。
- 在同一 Channel 上完成可重复的模型发现、参考目录采纳、绑定、验证和人工启用。
...
5. Channel 详情提供检测、凭据轮换、归档/恢复和模型或 Route 成员关系管理；凭据永不显示。渠道列表的
   “管理模型”和新建成功都进入 Channel 详情的模型页，页面 section 保存在 URL 中。
...
9. Channel 模型页顶部展示最近成功发现时间和发现、绑定、新增、待处理数量；视图分为“待处理”“已绑定”
   “全部发现”“运行表现”“发现历史”。清单列出上游模型 ID、本地模型、参考模型、上游状态、绑定状态、
   验证状态和操作。
...
| 上游模型发现、清单和验证 | [Gateway 渠道模型发现与清单](../../gateway/features/channel-model-inventory.md) | 授权运营人员；不含凭据和完整上游正文 | 任务轮询至终态，清单读取最近成功快照与当前绑定 |
...
- 新建 Channel 默认且固定为 disabled；创建成功进入 `models` section，并标明“发现 → 关联 → 验证 → 人工启用”流程。
- “重新发现”创建异步任务并轮询终态。失败时显示安全原因，但继续展示最近成功快照和已有绑定；没有成功快照时
  提供手工绑定入口。
- 未绑定的发现行优先提供本地精确匹配，其次提供已采纳参考模型、参考目录采纳并绑定或手工选择本地模型。
  参考目录存在多个候选时必须人工选择；采纳表单复用目录详情、元数据和能力提示。
- 新绑定显示为“已停用”。逐模型验证使用真实上游请求；当前验证成功时“启用”才可操作。验证、模型状态、
  Channel 状态和绑定状态分别显示，不能用一种状态替代另一种。
- 更换上游模型名必须先验证候选映射；只有验证通过才更新绑定，原绑定启停状态保持不变。
- “本次未发现”的已有绑定仍保留原状态和操作，页面不把它描述为上游已下架。
...
| 模型发现中 | 保持稳定表格结构并播报任务状态 | 等待轮询终态，不重复创建同类活动任务 |
| 模型发现失败但有快照 | 显示失败原因和“仍展示上次成功快照” | 修复后重新发现或继续处理已有快照 |
| 上游不支持列表 | 明示可以手工绑定，不把 404/405 当成不支持任何模型 | 选择本地模型并填写上游模型 ID，再执行逐模型验证 |
| 绑定停用 | 与模型停用、Channel 停用分别显示 | 当前映射验证成功且模型可用后人工启用绑定 |
...
- [x] 新建 Channel 固定停用并进入模型设置；已有 Channel 可从列表或详情重新发现和对账。
- [x] 模型页覆盖五个视图、任务轮询、成功快照保留、参考目录采纳并绑定和手工绑定兜底。
- [x] 模型、Channel、绑定和验证状态独立；启用或改映射必须提交当前成功验证证据。
